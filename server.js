...
app.post('/resend-verification', async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: 'Sport Scheduler: New Verification Code',
      html: `
...
app.post('/forgot-password', async (req, res) => {
  try {
    ...
    const resetLink = `${baseUrl}/reset-password.html?token=${token}&email=${email}`;
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          ...
          <h3 style="background-color: #f2f2f2; padding: 10px; display: inline-block; border-radius: 5px;">${verificationCode}</h3>
          <p>This code will expire in 15 minutes.</p>
...
app.post('/login', async (req, res) => {
  try {
    ...
    if (!user.isVerified) return res.status(400).json({ message: 'Email not verified. Please check your email for verification code.' });
...
// --- START SERVER ---
app.listen(PORT, () => {
  console.log(`🚀 Sport Scheduler Server running at http://localhost:${PORT}`);
